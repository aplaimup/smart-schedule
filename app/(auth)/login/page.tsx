...
export default function LoginPage() {
  ...
  const handleLogin = () => {
    // Simulasi set cookie login
    document.cookie = "smart-schedule-session=true; path=/; max-age=86400";
    toast.success("Berhasil masuk!");
    router.push("/dashboard");
    router.refresh(); // Memaksa re-render dengan status auth baru
  };
    ...
    <div className="min-h-screen bg-background flex flex-col justify-center items-center p-6">
      <div className="w-full max-w-md bg-card p-8 rounded-3xl shadow-lg border border-border/50 text-center">
        ...
        <div className="space-y-4 text-left">
          ...
          >
            Simulasi Masuk <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </Button>
        </div>
